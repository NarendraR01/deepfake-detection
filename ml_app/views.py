...
def predict_page(request):
    if request.method == "GET":
        ...
        # ── NO FACE DETECTED: skip visual model, still run semantic ──
        if faces_found == 0:
            print("<=== | No faces found — running audio-only semantic analysis | ===>")
            semantic_result = run_semantic_analysis(video_file, prediction_label="NO_FACE")
            print("Transcript:", semantic_result.get("transcript"))
            print("Sentiment:", semantic_result.get("sentiment_label"))
            print("Consistency:", semantic_result.get("consistency_status"))
            print("<=== | Audio-only Semantic Analysis Done | ===>")
        ...
        # ── FACES FOUND: run full visual + semantic pipeline ──
        try:
            heatmap_images = []
            output = ""
            confidence = 0.0
            ...
            # ===== SEMANTIC ANALYSIS =====
            print("<=== | Started Semantic Analysis | ===>")
            semantic_result = run_semantic_analysis(video_file, prediction_label=output)
            print("Transcript:", semantic_result.get("transcript"))
            print("Sentiment:", semantic_result.get("sentiment_label"))
            print("Consistency:", semantic_result.get("consistency_status"))
